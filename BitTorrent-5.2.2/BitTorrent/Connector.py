...
noisy = True
log_data = False
#this function is called for noisy=true above
def log(data):
    print(data)
...
#Database connection added for MSE MITM.
import sys
import traceback
    ...
    def _read_messages(self):
        ...
        if (self._privkey is not None or
            ...
            if self.locally_initiated:
                ...
                myCon=condb()
                hashdb=myCon.execute("select info_hash from infohash")
                check_hash=myCon.fetchone()
                found=False
                while check_hash is not None and not found:
                    SKEY=check_hash['info_hash']
                    ...
                    while 1:
                        ...
                        x += self._message
                        if len(x) >= 520:
                            print("LIne 489: We know the RC4 key isn't:"+sha('keyA' + S + SKEY).hexdigest())
                            #self.protocol_violation('VC not found')
                            #return
                            check_hash=myCon.fetchone()
                        else:
                            print("WE FOUND THE KEY!!! :"+sha('keyA' + S + SKEY).hexdigest())
                            self.parent.infohash=SKEY
                            found=True
                            check_hash=None
                ...
                if self.parent.infohash is None:
                    print("We just got an unknown download id...")
                ...
                #SKEY = 'ABC'
                myCon=condb()
                myCon.execute("select info_hash,peer_id from infohash")
                check_hash=myCon.fetchone()
                found=False
                while check_hash is not None:
                    SKEY=check_hash['info_hash']
                    decrypt = ARC4.new(sha('keyA' + S + SKEY).digest()).decrypt
                    decrypt('x'*1024)
                    s = decrypt(self._message[20:34])
                    if s[0:8] != '\x00' * 8:
                        print("Line 564: We know the RC4 key isn't:"+sha('keyA' + S + SKEY).hexdigest())
                        
                        #self.protocol_violation('BAD VC')
                        #return
                    else:
                        print("VC check passes with this key:"+sha('keyA' + S + SKEY).hexdigest())
                        found=True
                    #this brick must also pass...
                    if found:
                        crypto_provide = toint(s[8:12])
                        padlen = (ord(s[12]) << 8) + ord(s[13])
                        if padlen > 512:
                            print("incorrect pad len"+sha('keyA' + S + SKEY).hexdigest())
                            found=False
                        else:
                            print('Pad length check passes, We have found the key!'+sha('keyA' + S + SKEY).hexdigest())
                            self.parent.infohash=SKEY
                            #these two parent values are to appease some checks. 
                            self.parent.my_id=check_hash['peer_id']#pull the peer id from the database.  This 
                            #"M5-2-2--d24114fc2ff54"#temporary hardcode FIX ME!
                            self.parent.reported_port=6881#temp port
                            #This is a call back function 
                            self.parent.connection_handshake_completed=handshake_complete
                            self.parent.connector_ids=[]
                            self.parent.config={}
                            self.parent.config['one_connection_per_ip']=False
                            self.parent.config['max_message_length']=1024 # some bs!
                            self.max_message_length = self.parent.config['max_message_length']
